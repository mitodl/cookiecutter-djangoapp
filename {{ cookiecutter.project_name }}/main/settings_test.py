...
REQUIRED_SETTINGS = {
    ...
    "{{ cookiecutter.project_name|upper }}_BASE_URL": "http://localhost:8053",
}
...
class TestSettings(TestCase):
    ...
    def test_s3_settings(self):
        ...
        settings_vars = self.patch_settings(
            {**REQUIRED_SETTINGS, "{{ cookiecutter.project_name|upper }}_USE_S3": "False"}
        )
        self.assertNotEqual(
        ...
        with self.assertRaises(ImproperlyConfigured):
            self.patch_settings({"{{ cookiecutter.project_name|upper }}_USE_S3": "True"})
    ...
    def test_admin_settings(self):
        ...
        settings_vars = self.patch_settings(
            {**REQUIRED_SETTINGS, "{{ cookiecutter.project_name|upper }}_ADMIN_EMAIL": ""}
        )
        self.assertFalse(settings_vars.get("ADMINS", False))

        test_admin_email = "cuddle_bunnies@example.com"
        settings_vars = self.patch_settings(
            {**REQUIRED_SETTINGS, "{{ cookiecutter.project_name|upper }}_ADMIN_EMAIL": test_admin_email}
        )
    ...
    def test_db_ssl_enable(self):
        ...
        settings_vars = self.patch_settings(
            {**REQUIRED_SETTINGS, "{{ cookiecutter.project_name|upper }}_DB_DISABLE_SSL": "True"}
        )
        self.assertEqual(settings_vars["DATABASES"]["default"]["OPTIONS"], {})

        # Disable it
        settings_vars = self.patch_settings(
            {**REQUIRED_SETTINGS, "{{ cookiecutter.project_name|upper }}_DB_DISABLE_SSL": "False"}
        )
        self.assertEqual(
            settings_vars["DATABASES"]["default"]["OPTIONS"], {"sslmode": "require"}
        )
    ...
    def test_server_side_cursors_enabled(self):
        """DISABLE_SERVER_SIDE_CURSORS should be false if {{ cookiecutter.project_name }}_DB_DISABLE_SS_CURSORS is false"""
        settings_vars = self.patch_settings(
            {**REQUIRED_SETTINGS, "{{ cookiecutter.project_name|upper }}_DB_DISABLE_SS_CURSORS": "False"}
        )
        assert (
            settings_vars["DEFAULT_DATABASE_CONFIG"]["DISABLE_SERVER_SIDE_CURSORS"]
            is False
